...
exports.default = _default;
},{"d3":"../node_modules/d3/index.js"}],"data/df_age30.csv":[function(require,module,exports) {
module.exports = "/df_age30.88475158.csv";
},{}],"images2.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#images2').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
svg.append('rect').attr('width', width).attr('height', height).attr('opacity', 0);

var xPositionScale = _fisheye.default.scale(d3.scaleLinear).range([0, width]).focus(width / 2).distortion(6);

d3.csv(require('./data/df_age30.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  holders.append('text').text(function (d) {
    return 'Movie: ' + d['Movie Name'] + ' ' + '(' + d['Release Year'] + ')';
  }).attr('class', function (d) {
    var str = d['Movie Name'].replace(/'/g, '');
    return 'textelem' + str.replace(/\s+/g, '-').toLowerCase();
  }).classed('movie-text', true).attr('x', 10).attr('y', -50).attr('dx', function (d) {
    if (d['Movie Name'] === 'The Quiet American ') {
      return -100;
    } else {
      return 0;
    }
  }).attr('text-anchor', 'star').attr('fill', 'black').attr('opacity', 0);
  holders.append('text').text(function (d) {
    return d['Actor 1 Name'] + ' ' + '(' + d['Actor 1 Age'] + ')';
  }).attr('class', function (d) {
    var str = d['Movie Name'].replace(/'/g, '');
    return 'textelem' + str.replace(/\s+/g, '-').toLowerCase();
  }).classed('movie-text', true).attr('x', 10).attr('y', -30).attr('dx', function (d) {
    if (d['Movie Name'] === 'The Quiet American ') {
      return -100;
    } else {
      return 0;
  ...
  holders.append('text').text(function (d) {
    return d['Actor 2 Name'] + ' ' + '(' + d['Actor 2 Age'] + ')';
  }).attr('class', function (d) {
    var str = d['Movie Name'].replace(/'/g, '');
    console.log('textelem' + str.replace(/\s+/g, '-').toLowerCase());
    return 'textelem' + str.replace(/\s+/g, '-').toLowerCase();
  }).classed('movie-text', true).attr('x', 10).attr('y', -10).attr('dx', function (d) {
    if (d['Movie Name'] === 'The Quiet American ') {
      return -100;
    } else {
      return 0;
  ...
  }).classed('movie-text', true).classed('movie-text', true).attr('width', 190).attr('height', height).on('mouseover', function (d) {
    console.log(d);
    var str = d['Movie Name'].replace(/'/g, '');
    var class_selected = str.replace(/\s+/g, '-').toLowerCase();
    console.log(str);
    console.log(class_selected);
    d3.selectAll('.textelem' + class_selected).attr('opacity', 1); // d3.select('#movie-title').text(d['Movie Name'])
  ...
  holders.append('rect').attr('stroke', 'none').attr('height', height).attr('width', 150).attr('fill', 'none');
...
},{"d3":"../node_modules/d3/index.js","./fisheye":"fisheye.js","d3-scale":"../node_modules/d3-scale/src/index.js","./data/df_age30.csv":"data/df_age30.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","images2.js"], null)
//# sourceMappingURL=/images2.852e3916.map