...
var colorScale = d3.scaleOrdinal().range(['#4b1803', '#726737', '#d9a746', '#bb5e22', '#739aa6']);
var xPositionScale = d3.scaleLinear().range([0, width]).domain([15, 70]);
var yPositionScale = d3.scalePoint().range([height - 50, 180]);
var xPostion = d3.scalePoint().range([0, width]);
var div = d3.select('body').append('div').attr('class', 'tooltip').style('opacity', 0);
d3.csv(require('./data/top_directors_withimg2.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  var nested = d3.nest().key(function (d) {
    return d.Director;
  }).entries(datapoints); // sort nested data
  ...
  xPostion.domain(names); // add director's images to circles

  defs.selectAll('.director-pattern').data(datapoints).enter().append('pattern').attr('class', 'director-pattern').attr('id', function (d) {
  ...
  var labelGroup = svg.append('g').attr('id', 'label').attr('transform', 'translate(-30, 0)'); // add director images

  labelGroup.selectAll('.label-circle').data(nested).enter().append('circle').attr('r', 40).attr('cx', function (d) {
    return xPostion(d.key);
  }).attr('cy', 20).attr('fill', function (d) {
    return 'url(#' + d.key.toLowerCase().replace(/ /g, '') + ')';
  }).attr('class', function (d) {
    return d.key.toLowerCase().replace(' ', '');
  }).classed('labels', true).attr('opacity', 0.9).style('pointer-events', 'none'); // add directors name under image

  labelGroup.selectAll('.label-text').data(nested).enter().append('text').text(function (d) {
    return d.key;
  }).attr('font-size', 14).attr('font-weight', 'bold').attr('text-anchor', 'middle').attr('opacity', 0.9).attr('fill', function (d) {
    return colorScale(d.key);
  }).attr('y', 80).classed('labels-text', true).attr('x', function (d) {
    return xPostion(d.key);
  }).attr('class', function (d) {
    return 'text' + d.key.toLowerCase().replace(' ', '');
  }).classed('labels-text', true); // // draw the average line and hide it
  ...
  var xAxis = d3.axisBottom(xPositionScale);
  svg.append('g').transition().attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis).style('visibility', 'hidden');
  var yAxis = d3.axisLeft(yPositionScale).tickSize(0);
  svg.append('g').transition().attr('class', 'axis y-axis').call(yAxis).style('visibility', 'hidden'); // svg.selectAll('.y-axis path')
  // filter the datapoints for each director

  var dir1Datapoints = datapoints.filter(function (d) {
    return d.Director === 'Woody Allen';
  });
  var dir2Datapoints = datapoints.filter(function (d) {
    return d.Director === 'John Glen';
  });
  var dir3Datapoints = datapoints.filter(function (d) {
    return d.Director === 'Lewis Gilbert';
  });
  var dir4Datapoints = datapoints.filter(function (d) {
    return d.Director === 'Joel Coen';
  });
  var dir5Datapoints = datapoints.filter(function (d) {
    return d.Director === 'Jonathan Lynn';
  }); // Scrollytelling!
  ...
  d3.select('#director5').on('stepin', function () {
    // reset the yAxis domain with new movie names
    var dir5MovieName = dir5Datapoints.map(function (d) {
      return d['Movie Name'];
    });
    yPositionScale.domain(dir5MovieName).range([(height - 45) / 2, 180]); // remove circles and axis from previous director

    svg.selectAll('#act1').remove();
    svg.selectAll('#act2').remove();
    svg.selectAll('#bar').remove();
    svg.selectAll('.y-axis').remove();
    svg.selectAll('.x-axis').remove(); // call the new xAxis

    svg.append('g').transition().attr('class', 'axis x-axis').attr('transform', 'translate(0, ' + (height - 12) / 2 + ')').call(xAxis);
    var yAxis = d3.axisLeft(yPositionScale);
    svg.append('g').attr('class', 'axis y-axis').call(yAxis);
    svg.selectAll('.y-axis path').attr('stroke', 'none');
    svg.selectAll('.domain').remove(); // console.log(xPostion.domain())
    // place circles for actor1

    svg.selectAll('.act1').data(dir5Datapoints).enter().append('circle').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 1 Gender'];
    }).attr('id', 'act1').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('cx', function (d) {
      var age = +d['Actor 1 Age'];
      return xPositionScale(age);
    }).attr('cy', function (d) {
      return yPositionScale(d['Movie Name']);
    }).attr('r', 6).attr('fill', function (d) {
      return colorScale(d.Director);
    }); // place circles for actor 2

    svg.selectAll('.act2').data(dir5Datapoints).enter().append('circle').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 2 Gender'];
    }).attr('id', 'act2').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('cx', function (d) {
    ...
    svg.selectAll('.bar').data(dir5Datapoints).enter().append('line').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'];
    }).attr('id', 'bar').attr('x1', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('x2', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('y1', 25).attr('y2', 25).transition().attr('x1', function (d) {
      return xPositionScale(+d['Actor 1 Age']);
    }).attr('y1', function (d) {
      return yPositionScale(d['Movie Name']);
    }).attr('x2', function (d) {
      return xPositionScale(+d['Actor 2 Age']);
    }).attr('y2', function (d) {
      return yPositionScale(d['Movie Name']);
    }).attr('stroke-width', 1).attr('stroke', function (d) {
      return colorScale(d.Director);
    }).attr('opacity', 0.5); //* MOUSEOVER PART *//

    svg.selectAll('circle').on('mouseover', function (d) {
      // console.log(d)
      var className = 'dif' + d['dif'] + d['Release Year'];
      d3.select('circle.' + className).raise().transition().attr('r', 10).attr('stroke-width', 1).attr('fill', '#3C5A6A');
      d3.select('circle.' + className).raise().transition().attr('r', 10).attr('stroke-width', 1).attr('fill', '#BC5E21');
      d3.selectAll('line.' + className).transition().attr('stroke-width', '3');
      div.transition().style('opacity', 0.9);
      div.html(d['Actor 2 Name'] + ': ' + d['Actor 2 Age'] + '<br/>' + d['Actor 1 Name'] + ': ' + d['Actor 1 Age'] + '<br/>' + 'Release Year: ' + d['Release Year']).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 28 + 'px');
      ...
      d3.selectAll('circle.' + className).transition().attr('stroke', 'none').attr('r', 6).attr('stroke-width', 0).attr('fill', function (d) {
        return colorScale(d.Director);
      });
      d3.selectAll('line.' + className).transition().attr('stroke-width', '1');
    });
    ...
    svg.selectAll('.jonathanlynn').transition().attr('opacity', 0.9);
    svg.selectAll('.labels-text').transition().attr('opacity', 0.2).attr('fill', function (d) {
      return colorScale(d.key);
    });
    svg.selectAll('.textjonathanlynn').transition().attr('opacity', 0.9);
  });
  d3.select('#director4').on('stepin', function () {
    var dir4MovieName = dir4Datapoints.map(function (d) {
      return d['Movie Name'];
    });
    ...
    svg.append('g').transition().attr('class', 'axis x-axis').attr('transform', 'translate(0, ' + (height - 50) / 1.5 + ')').call(xAxis);
    svg.selectAll('.domain').remove();
    svg.selectAll('.act1').data(dir4Datapoints).enter().append('circle').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 1 Gender'];
    }).attr('id', 'act1').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('cx', function (d) {
    ...
    svg.selectAll('.act2').data(dir4Datapoints).enter().append('circle').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 2 Gender'];
    }).attr('id', 'act2').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('cx', function (d) {
    ...
    svg.selectAll('.bar').data(dir4Datapoints).enter().append('line').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'];
    }).attr('id', 'bar').attr('x1', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('x2', function (d) {
    ...
    svg.selectAll('circle').on('mouseover', function (d) {
      ...
      d3.select('circle.' + className).raise().transition().attr('r', 10).attr('stroke-width', 1).attr('fill', '#3C5A6A');
      d3.select('circle.' + className).raise().transition().attr('r', 10).attr('stroke-width', 1).attr('fill', '#BC5E21');
      d3.selectAll('line.' + className).transition().attr('stroke-width', '3'); // div.transition().style('opacity', 0.9)
    ...
    svg.selectAll('.joelcoen').transition().attr('opacity', 0.9);
    svg.selectAll('.labels-text').transition().attr('opacity', 0.2); // .attr('fill', d => colorScale(d.key))

    svg.selectAll('.textjoelcoen').transition().attr('opacity', 0.9);
    var yAxis = d3.axisLeft(yPositionScale);
    svg.append('g').attr('class', 'axis y-axis').call(yAxis);
    svg.selectAll('.y-axis path').attr('stroke', 'none');
  });
  d3.select('#director3').on('stepin', function () {
    var dir3MovieName = dir3Datapoints.map(function (d) {
      return d['Movie Name'];
    });
    ...
    svg.append('g').transition().attr('class', 'axis x-axis').attr('transform', 'translate(0, ' + (height - 12) / 2 + ')').call(xAxis);
    svg.selectAll('.domain').remove();
    svg.selectAll('.act1').data(dir3Datapoints).enter().append('circle').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 1 Gender'];
    }).attr('id', 'act1').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('cx', function (d) {
    ...
    svg.selectAll('.act2').data(dir3Datapoints).enter().append('circle').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 2 Gender'];
    }).attr('id', 'act2').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('cx', function (d) {
    ...
    svg.selectAll('.bar').data(dir3Datapoints).enter().append('line').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'];
    }).attr('id', 'bar').attr('x1', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('x2', function (d) {
    ...
    svg.selectAll('.lewisgilbert').transition().attr('opacity', 0.9);
    svg.selectAll('.labels-text').transition().attr('opacity', 0.2); // .attr('fill', d => colorScale(d.key))

    svg.selectAll('.textlewisgilbert').transition().attr('opacity', 0.9);
    var yAxis = d3.axisLeft(yPositionScale);
    svg.append('g').attr('class', 'axis y-axis').call(yAxis);
    svg.selectAll('.y-axis path').attr('stroke', 'none');
  });
  d3.select('#director2').on('stepin', function () {
    var dir2MovieName = dir2Datapoints.map(function (d) {
      return d['Movie Name'];
    });
    yPositionScale.domain(dir2MovieName).range([(height - 50) / 1.5, 180]);
    svg.selectAll('.x-axis').remove();
    svg.append('g').transition().attr('class', 'axis x-axis').attr('transform', 'translate(0, ' + (height - 20) / 1.5 + ')').call(xAxis); // svg
    //   .selectAll('.x-axis')
    ...
    svg.selectAll('#bar').remove();
    svg.selectAll('.y-axis').remove();
    svg.selectAll('.act1').data(dir2Datapoints).enter().append('circle').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 1 Gender'];
    }).attr('id', 'act1').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('cx', function (d) {
    ...
    svg.selectAll('.act2').data(dir2Datapoints).enter().append('circle').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 2 Gender'];
    }).attr('id', 'act2').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('cx', function (d) {
    ...
    svg.selectAll('.bar').data(dir2Datapoints).enter().append('line').attr('x1', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('x2', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('y1', 25).attr('y2', 25).transition().attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'];
    }).attr('id', 'bar').attr('x1', function (d) {
      return xPositionScale(+d['Actor 1 Age']);
    }).attr('y1', function (d) {
    ...
    svg.selectAll('.johnglen').transition().attr('opacity', 0.9);
    svg.selectAll('.labels-text').transition().attr('opacity', 0.2); // .attr('fill', d => colorScale(d.key))

    svg.selectAll('.textjohnglen').transition().attr('opacity', 0.9);
    var yAxis = d3.axisLeft(yPositionScale);
    svg.append('g').attr('class', 'axis y-axis').call(yAxis);
    svg.selectAll('.y-axis path').attr('stroke', 'none');
  });
  d3.select('#director1').on('stepin', function () {
    var dir1MovieName = dir1Datapoints.map(function (d) {
      return d['Movie Name'];
    });
    ...
    svg.selectAll('.x-axis').remove();
    var xAxis = d3.axisBottom(xPositionScale);
    svg.append('g').transition().attr('class', 'axis x-axis').attr('transform', 'translate(0, ' + (height - 30) + ')').call(xAxis);
    svg.selectAll('.domain').remove();
    svg.selectAll('.act1').data(dir1Datapoints).enter().append('circle').attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 1 Gender'];
    }).attr('id', 'act1').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('cx', function (d) {
    ...
    svg.selectAll('.act2').data(dir1Datapoints).enter().append('circle').attr('cx', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('cy', 25).transition().attr('class', function (d) {
      return 'dif' + d['dif'] + d['Release Year'] + ' ' + d['Actor 2 Gender'];
    }).attr('id', 'act2').attr('cx', function (d) {
      var age = +d['Actor 2 Age'];
      return xPositionScale(age);
    ...
    svg.selectAll('.bar').data(dir1Datapoints).enter().append('line').attr('x1', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('x2', function (d) {
      return xPostion(d.Director) - 25;
    }).attr('y1', 25).attr('y2', 25).transition().attr('class', function (d) {
    ...
    svg.selectAll('circle').on('mouseover', function (d) {
      ...
      var className = 'dif' + d['dif'] + d['Release Year'];
      div.transition().style('opacity', 0);
      d3.selectAll('circle.' + className).transition().attr('stroke', 'none').attr('r', 6).attr('fill', function (d) {
        return colorScale(d.Director);
      });
      d3.selectAll('line.' + className).transition().attr('stroke-width', '1').attr('stroke', function (d) {
        return colorScale(d.Director);
      });
    ...
    svg.selectAll('.woodyallen').transition().attr('opacity', 0.9);
    svg.selectAll('.labels-text').transition().attr('opacity', 0.2); // .attr('fill', d => colorScale(d.key))

    svg.selectAll('.textwoodyallen').transition().attr('opacity', 0.9);
    var yAxis = d3.axisLeft(yPositionScale);
    svg.append('g').attr('class', 'axis y-axis').transition().call(yAxis);
    svg.selectAll('.y-axis path').attr('stroke', 'none');
  });
}
},{"d3":"../node_modules/d3/index.js","./data/top_directors_withimg2.csv":"data/top_directors_withimg2.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
