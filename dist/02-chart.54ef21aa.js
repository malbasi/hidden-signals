...
module.exports = "/age_gap.072a04f1.csv";
...
var xPositionScale = d3.scaleLinear().range([0, width]).domain([15, 80]);

var yPositionScale = d3.scaleLinear().range([height, 0]).domain([15, 80]);
...
function ready(datapoints) {
  // Add dots for each
  svg.selectAll('circles').data(datapoints).enter().append('circle').attr('class', 'couples').attr('id', function (d) {
    var str = d.Director;
    ...
    div.html(d['Actor 1 Name'].bold() + '<br/>' + ' ♡ ' + '<br/>' + d['Actor 2 Name'].bold() + '<br/>' + 'Age difference: ' + d['Age Difference'] + '<br/>' + 'Movie: ' + d['Movie Name']).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 28 + 'px');
  }).on('mouseout', function (d) {
    div.transition().duration(500).style('opacity', 0);
  });

  // add a line for matching age
  svg.append('line').attr('x1', 0).attr('y1', height).attr('x2', width).attr('y2', 0).attr('stroke', '#4B1803').attr('stroke-width', 1.8).attr('opacity', 0.7).lower();

  // add text. This is garbage atm, but i forget how translate rotate works
  svg.append('text').text('Actors are the same age').attr('font-weight', 'bold').attr('fill', '#4B1803').attr('x', 350).attr('y', 450).attr('text-anchor', 'middle').attr('transform', 'rotate(-45)');

  /* Set up axes */
  var xAxis = d3.axisBottom(xPositionScale).tickSize(-height);
  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis).attr('stroke-width', 0.1).attr('stroke', 'gray').lower();

  var yAxis = d3.axisLeft(yPositionScale).tickSize(-width);
  svg.append('g').attr('class', 'axis y-axis').call(yAxis)
  // .attr('stroke-dasharray', '2,4')
  .attr('stroke-width', 0.1).attr('stroke', 'gray').lower();
  ...
  // reset circles
  d3.select('#top2').on('stepin', function () {
...
},{"d3":"../node_modules/d3/index.js","./data/age_gap.csv":"data/age_gap.csv"}],"../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/02-chart.54ef21aa.map