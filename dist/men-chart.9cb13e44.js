...
var container = d3.select('#men-chart'); // Create your scales

var xPositionScale = d3.scaleLinear().domain([1983, 2018]).range([0, width]);
var yPositionScale = d3.scaleLinear().domain([0, 60]).range([height, 0]); // Creating line functions

var lineMale = d3.line().x(function (d) {
  return xPositionScale(+d.ReleaseYear);
}).y(function (d) {
  return yPositionScale(+d['Actor 1 Age']);
});
var lineFemale = d3.line().x(function (d) {
  return (// console.log(+d['Release Year'])
...
d3.csv(require('./data/men.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  var nested = d3.nest().key(function (d) {
    return d['Actor 1 Name'];
  }).sortValues(function (a, b) {
    return +a.ReleaseYear - +b.ReleaseYear;
  }).entries(datapoints); // .sort(function (a, b) { return d3.descending(a.values.ReleaseYear, b.values.ReleaseYear) })

  console.log('nested looks like', nested); // console.log('nested data looks like', nested)

  container.selectAll('.actor-svg').data(nested).enter().append('svg').attr('class', 'actor-svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    // console.log('d looks like', d)
    var svg = d3.select(this); // console.log('d.values looks like', d.values)

    svg.append('path').datum(d.values).attr('d', lineMale).attr('stroke', '#3C5A6A').attr('stroke-width', 2.5).attr('fill', 'none');
    svg.append('path').datum(d.values).attr('d', lineFemale).attr('stroke', '#BC5E21').attr('stroke-width', 2.5).attr('fill', 'none');
    svg.append('text').text(d.key).attr('x', width / 2).attr('y', 0).attr('font-size', 14).attr('text-anchor', 'middle'); // .attr("font-weight", 'bold')
    // .attr('fill', '#3C5A6A')
...
},{"d3":"../node_modules/d3/index.js","./data/men.csv":"data/men.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","men-chart.js"], null)
//# sourceMappingURL=/men-chart.9cb13e44.map